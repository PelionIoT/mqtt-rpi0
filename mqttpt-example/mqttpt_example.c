...
int mqttpt_receive_write_handler(const connection_id_t *connection,
                                 ...
                                 void* userdata);
void ipso_create_set_point_mqtt(connection_id_t connection_id,
...
{
    pt_status_t status = PT_STATUS_SUCCESS;
    int sensor_data_len = strlen(data);
    uint8_t *sensor_data = malloc(sensor_data_len);
    if (sensor_data == NULL) {
        tr_err("Could not allocate temperature_data");
        return;
    }
    memcpy(sensor_data, data, sensor_data_len);
                                         ...
                                         const int payload_len)
{
    ...
    if(strcmp(device_type,"envSensor")==0)
    {
		int object_id = GENERIC_SENSOR;
		int object_instance = 0,index=0;
		int resource_id = 5700;
		char *value = device_status;
		char final_device_name[200];
...
/*            else if(strcmp(device_name_env,"PM")==0)
        ...
        if (payload_field_size > 0) {
				...
				char* deveui_ctx = strdup(final_device_name);
				// If device has been registered, then just write the new values
				if (mqttpt_device_exists(final_device_name)) {
 ...
 *
 */
#define MQTT_TOPIC_OFFSET_COUNT 10
void mqttpt_handle_message(struct mosquitto *mosq, char *topic, char *payload, int payload_len)
{
    ...
    sem_getvalue(&mqttpt_translator_started, &mqtt_pt_started);
    if (mqtt_pt_started == 0) {
        mqttpt_start_translator(mosq);
